...
const { generateOrderProducts } = require("./orderData.js");
...
const loadProducts = async () => {
  products = await Product.find({})
    .populate("category")
    .populate({ path: "tiers", populate: "ingredients" });
};
  ...
  for (const user of users) {
    ...
    for (let index = 0; index < 5; index += 1) {
      const status = hasAddedActiveOrder
        ? completedStatuses[
            Math.floor(Math.random() * completedStatuses.length)
          ]
      ...
      const { products: finalProducts, totalPrice } =
        await generateOrderProducts(randomProducts);
